...
const ScoreBoard = ({totalScore, currentRound, rounds, timeLeft, timer}) => {
  return(
    <div id="score-board" className="col-span-2 grid grid-cols-2 gap-x-3 p-2 pt-5 text-sm mt-2 justify-items-start md:grid-cols-5 md:p-0 md:mx-3 md:text-xl">
      {timer ? 
        <>
          <div className="col-span-2 md:col-span-5 w-full h-4">
            <ProgressBar percentage={((timeLeft/timer)*100).toFixed(2)} />
          </div>
          <div className="md:col-span-2">Time left</div><div className="md:col-span-3">{timeLeft} s</div>
        </> : null}
